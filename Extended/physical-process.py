...
class RawWaterTank(PLC):
	...
	def main_loop(self):
		...
		while(count <= PP_SAMPLES):
			...
			logging.debug('Q2 value read from database is %s ', self.Q2)
			...
			logging.debug('lit101 is set to %s ', self.l[0])
			logging.debug('lit102 is set to %s ', self.l[1])
			logging.debug('lit103 is set to %s ', self.l[2])
